...
class TestLink {
  ...
  connect() {
    ...
    this.rpcClient = this.secure ? xmlrpc.createSecureClient(options) : xmlrpc.createClient(options);
  }
   ...
   */
  createBuild(options) {
    checkMandatoryFields(options, ["testplanid", "buildname", "buildnotes", "active", "open", "releasedate"]);
  ...
  createTestPlan(options) {
    checkMandatoryFields(options, ["testplanname", ["testprojectname", "prefix"]]);
  ...
  assignRequirements(options) {
    checkMandatoryFields(options, [
      "testcaseexternalid",
      "testprojectid",
      { requirements: ["req_spec", "requirements"] }
    ]);
   ...
   *
   * @param {object}  options Options
   * @param {int}     options.id Project ID
   *
   * @returns {object}  result Result object.
   */
  getProjectTestPlans(options) {
    checkMandatoryFields(options, ["testprojectid"]);
    //checkMandatoryFields(options, ["id"]);
   ...
   * @param {string}  [options.testcaseid] Test Case id. If not present, testcaseexternalid must be present.
   * @param {string}  [options.testcaseexternalid] Test Case external id. If not present, testcaseid must be present.
   * @param {int}     [options.platformid] Platform id number. If this and platformname are not specified, the latest will be used.
   * @param {string}  [options.platformname] Platform name. If this and platformname are not specified, the latest will be used.
   * @param {int}     [options.buildid] Build id number. If this and buildname are not specified, the latest build will be used.
   * @param {string}  [options.buildname] Build name. If this and buildid are not specified, the latest build will be used.
   * @param {object}  [options.options]
  ...
  getLastExecutionResult(options) {
    checkMandatoryFields(options, ["tplanid", ["testcaseid", "testcaseexternalid"], ["buildid", "buildname"]]);
  ...
  _performRequest(method, options = {}) {
    if (options.hasOwnProperty("apiKey")) options.devKey = options.apiKey;
    else if (!options.hasOwnProperty("devKey")) options.devKey = this.apiKey;

    return new Promise((resolve, reject) => {
      this.rpcClient.methodCall("tl." + method, [options], (error, value) => {
        if (error) return reject(error);
        else if (value.status_ok === 0) return reject(value.msg);
        else if (Array.isArray(value)) {
          for (let v of value) {
            if (v.hasOwnProperty("code")) return reject("[" + v.code + "] " + v.message);
          }
        }
  ...
  if (Array.isArray(options)) {
    ...
    for (let field of mandatoryFields) {
      if (Array.isArray(field)) {
        if (!_checkMandatoryFieldsArray(options, field)) throw Error("Expected one of this fields: [" + field + "].");
      } else if (typeof field === "object") _checkMandatoryFieldsObject(options, field);
      else if (!(field in options)) throw Error("Field '" + field + "' not present.");
    }
  }
}
...
function _checkMandatoryFieldsArray(options, field) {
  for (let subfield of field) {
    if (subfield in options) return true;
  }
...
function _checkMandatoryFieldsObject(options, field) {
  for (let fieldKey in field) {
    if (fieldKey in options) checkMandatoryFields(options[fieldKey], field[fieldKey]);
    else throw Error("Field '" + fieldKey + "' not present.");
  }
}
